...
class TFDump(object):
    ...
    def load_data(self):
        with open(PATH + 'data/amazon/dataset.pkl', 'rb') as f:
            self.train = pickle.load( f )
            self.valid = pickle.load( f )

        with open(PATH + 'data/amazon/remap.pkl', 'rb') as f:
            _ = pickle.load( f )
    ...
    def dump(self, data, type):
        with tf.python_io.TFRecordWriter(PATH + '/data/amazon/amazon_{}.tfrecords'.format(type)) as writer:
