...
CSV_COLUMN_DEFAULTS = [[-1], [''], [-1], [''], [-1], [''], [''], [''], [''], [''], [-1], [-1], [-1], [''], ['']]
...
def get_numeric_feature_range():
    train = pd.read_csv('toy_data/adult.data', header=None, names=CSV_COLUMNS)[['age', 'education_num', 'capital_gain', 'capital_loss', 'hours_per_week']]
    test = pd.read_csv('toy_data/adult.test', header=None, names=CSV_COLUMNS)[['age', 'education_num', 'capital_gain', 'capital_loss', 'hours_per_week']]
...
def build_wide_columns():
    n_range = get_numeric_feature_range()
    base_columns = [
...
def feature_columns_config_demo():
    # categorical_column
